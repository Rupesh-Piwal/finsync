...
import { MonthlyStats, Stats, TransactionDue } from "@/types";
...
// Utility functions
function isTransactionDue(transaction: TransactionDue): boolean {
  // If no lastProcessed or nextRecurringDate, treat as not due (or customize logic)
  if (!transaction.lastProcessed || !transaction.nextRecurringDate) {
    return false;
  }

  const today = new Date();
  const nextDue = new Date(transaction.nextRecurringDate); // safe now
...
async function getMonthlyStats(
  ...
  return transactions.reduce<MonthlyStats>(
    (stats, t) => {
      const amount = t.amount.toNumber(); // safely convert Prisma Decimal
      if (t.type === "EXPENSE") {
